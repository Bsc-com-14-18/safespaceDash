    ...
    const fetchHandlerCaseType = async () => {
      try {
        ...
        const querySnapshot = await getDocs(
          query(collection(db, 'hanlderCaseType'), where('handlerId', '==', handlerId))
        );
        ...
        const casesQuery = query(
          collection(db, 'cases'),
          where('type', '==', handlerCaseType),
          where('status', '==', 'not resolved')
        );
        const unsubscribe = onSnapshot(casesQuery, (snapshot) => {
          const cases = snapshot.docs.map((doc) => {
            const caseData = doc.data();
            const formattedTimestamp = new Date(caseData.timestamp).toLocaleString('en-US', {
              timeZone: 'Europe/Paris'
            }); // Adjust the timeZone as needed
            const formattedDateSent = new Date(caseData.dateSent).toLocaleString('en-US', {
              timeZone: 'Europe/Paris'
            }); // Adjust the timeZone as needed
